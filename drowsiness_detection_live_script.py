...
model = load_model('models/harshali.h5')
...
while(True):
    ...
    for (x,y,w,h) in right_eye:
        ...
        right_eye_pred = model.predict_step(right_one)
        if(np.argmax(right_eye_pred[0]) == 1):
            labels = 'Open' 
        if(np.argmax(right_eye_pred[0])==0):
            labels = 'Closed'
        break
    ...
    for (x,y,w,h) in left_eye:
        ...
        left_eye_pred = model.predict_step(left_one)
        if(np.argmax(left_eye_pred[0]) == 1):
            labels ='Open'
        if(np.argmax(left_eye_pred[0]) == 0):
            labels ='Closed'
        break

    if (np.argmax(left_eye_pred[0])==0) and (np.argmax(right_eye_pred[0])==0):
        time += 1
